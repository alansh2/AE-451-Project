...
twist = linspace(3,-1,Nhalf+1);  % twist distribution on ys (deg)
% twist = 0.6*(2/pi^2*sqrt(1-ys.^2)+1/pi/8)*180/pi;
chord = 4*sqrt(1-ys.^2);         % chord distribution on ys
% chord = linspace(5,3,Nhalf+1);
    ...
    for i = 1:N
        ...
        if i == j
            vij(i,j,:) = cross(uinf,r2)/(R2*(R2-dot(uinf,r2))) +...
                ...
                cross(uinf,r1)/(R1*(R1-dot(uinf,r2)));
            un(i,:) = cross(r2,r1);
            ua(i,:) = r1 - 0.5*dl(i,:);
        else
            vij(i,j,:) = cross(uinf,r2)/(R2*(R2-dot(uinf,r2))) -...
